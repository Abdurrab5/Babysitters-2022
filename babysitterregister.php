...
if ($id > 0) {
  $query = "SELECT * FROM babysitter where babysitter_id='$id'   ";
  ...
  <div>
    ...
    if (isset($_POST['submit'])) {
      ...
      if ($id > 0) {
        ...
        $query = "update babysitter set first_name='$fname',last_name='$lname',username='$username',email='$email',password='$password',address='$address',gender='$gender',phone='$phone' where babysitter_id='$id'";

        $result = mysqli_query($link, $query);
        alert("user update successfuly.");

        if ($header == 'babysitter') {
        ...
        $query = "INSERT into babysitter(username,first_name,last_name,email,password,gender,address,phone) VALUES";
        $query .= "('$username ','$fname','$lname','$email','$password','$gender','$address','$phone')";
        $result = mysqli_query($link, $query);
        if (mysqli_insert_id($link)) {
          alert("babysitter registered successfuly.");

          redirect_to("babysitterlogin.php");
        } else {
    ...
    <div class="container" id="form">
      ...
      ?>
        <h3> Babysitter Registeration</h3>
      <?php
      }
      ?>
